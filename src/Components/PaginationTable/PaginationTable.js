...
const PaginationTable = (props) => {
    ...
    const onDragEnd = (result) => {
        ...
        reorderColumnsHandler(result.source.index - 1, result.destination.index - 1)
    }
    ...
    const handleOnMouseMove = (e) => {
        if (isResizing.current >= 0 && columnRefs[isResizing.current].current) {
            const parentLeftPoint = columnRefs[
                isResizing.current
    ...
    const actualTableWidth = Math.min(entireGridWidth, parentBoxRef.current?.offsetWidth || 0) + (anableSelectRows ? 49 : 0) + 20;

    return (
        <Box style={{ position: 'relative', height: `${tableHeight}vh` }} className={classes.root} ref={parentBoxRef}>
            {isLoading &&
                <div style={{ zIndex: 2, position: 'absolute', width: 400, top: '40%', left: '40%' }}>
                    <LinearProgress />
                </div>
            }
            {/* <TableContainer style={{ height: '58vh', width: actualTableWidth }} component={Paper}> */}
            <TableContainer style={{ width: actualTableWidth, height: `${tableHeight-7}vh` }} className={classes.tableContainer} component={Paper}>
                <Table stickyHeader aria-label="sticky table" className={classes.table} size='small'>
                        ...
                        <DragDropContext onDragEnd={onDragEnd}>
                            <Droppable droppableId="droppable" direction="horizontal">
                                {(provided, snapshot) => (
                                    ...
                                    >
                                        {anableSelectRows && (
                                            <TableCell padding="checkbox" className={classes.headerCell} style={{ width: 45 }}>
                                                <Checkbox
                                        ...
                                        {columns.filter(f => !f.hidden).map((col, index) => {
                                            if (col.field === 'rowNumber') {
                                                return (
                                                    <TableCell
                                                        ...
                                                        size='small'
                                                        variant='head'
                                                        className={clsx(classes.tableRightBorder, classes.tableCell, classes.headerCell)}
                                                        style={{ width: col?.width ?? "auto" }}

                                                    >
                                                        <Typography className={classes.td} component='span'>#</Typography>
                                                    </TableCell>
                                                )
                                            }
                                            return (
                                                    ...
                                                    {(provided, snapshot) => (
                                                        ...
                                                        >
                                                            <TableCell
                                                                ...
                                                                size='small'
                                                                variant='head'
                                                                //className={clsx(classes.tableRightBorder, classes.tableCell, 'resizable')}
                                                                className={clsx(classes.tableRightBorder, classes.tableCell, classes.headerCell, 'resizable')}
                                                                style={{ width: col?.width ?? "auto" }}

                                                            >
                                                                <TableSortLabel
                                                                    active={sort[0] && sort[0].columnName === col.field}
                                                                ...
                                                                >
                                                                    <Box
                                                                        ...
                                                                        className={classes.td}
                                                                    >
                                                                        <Typography className={classes.td} component='span'>{col.field.replaceAll('"', '')}</Typography>
                                                                    </Box>
                                                                </TableSortLabel>
                                                                <div
                    ...
                    <TableBody>
                            ...
                            <TableRow style={{ position: 'sticky', top: 33 }}>
                                {anableSelectRows &&
                                    <TableCell
                                        ...
                                        variant='head'
                                        padding='none'
                                        className={clsx(classes.tableRightBorder, classes.numCell)}
                                    ></TableCell>
                                }
                                {columns.filter(f => !f.hidden).map((col, index) => {
                                    if (col.field === 'rowNumber') {
                                    ...
                                    return (
                                        ...
                                        >
                                            <Filter
                                                columnType={col.type}
                                                filterValue={filter && filter.find(f => f.columnName === col.field).filterText}
                                                filterTypeValue={filter && filter.find(f => f.columnName === col.field).filterType}
                                                onChange={handleFilter}
                                                clearFilter={props.clearFilter}
                                                columnName={col.field}
                                                clearDisabled={filter && filter.find(f => f.columnName === col.field).filterText !== '' || filter && ['IS NULL', 'IS NOT NULL', 'EMPTY'].includes(filter.find(f => f.columnName === col.field).filterType)}
                        ...
                        {innerRows && innerRows.map((record, indexRow) => {
                            ...
                            return (
                                ...
                                >
                                    {anableSelectRows && (
                                        <TableCell padding="checkbox" className={classes.numCell}>
                                            <Checkbox
                                                checked={isSelected}
                                                onChange={(event) =>
                                    ...
                                    {columns.filter(f => !f.hidden).map((col, index) => {
                                        let dataItem = record[col.field && col.field.replaceAll('"', '').toUpperCase()]
                                        ...
                                        if (col.field === 'rowNumber') {
                                            return (
                                                <TableCell
                                                    ...
                                                    padding='normal'
                                                    variant='body'
                                                    className={clsx(classes.tableRightBorder, classes.tableCell, classes.numCell)}
                                                >
                                        ...
                                        return (
                                            ...
                                            >
                                                <TableCell
                                                    ...
                                                    padding='normal'
                                                    variant='body'
                                                    className={clsx(classes.tableRightBorder, classes.tableCell)}
                                                    onContextMenu={(e) => copyCellValue(e, dataItem)}
                                                >
                                                    <Typography className={classes.td}>{dataItem}</Typography>
                                                </TableCell>
                                            </Tooltip>
                                        )
                                    })}
            ...
            <TablePagination
                ...
                count={totalRowsCountUpdated}        //filtered.length
